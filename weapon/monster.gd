...
func apply_elemental_status(element_type: int, incoming_damage: float, active_combo: int, current_bullet: Projectile):
	...
	match element_type:
		...
		2: # --- ICE STATUS ---
			# --- FIX: Replaced 'Global' with 'LoadoutManager' and added property safety checks ---
			if active_combo == ComboManager.ComboID.RIFLE_BOUNCE_FREEZE:
				if current_bullet and LoadoutManager.active_bullet_profile:
					var max_bounce = LoadoutManager.active_bullet_profile.max_bounce
					if "current_bounce_left" in current_bullet and current_bullet.current_bounce_left < max_bounce:
						freeze_target(3.0)
						return
